...
?>
    ...
    <?php $form = ActiveForm::begin(['action' => '/despesa/create']); ?>
                            <div class="col-sm-6">
                                <label>Categoria</label>
							 <?= $form->field($model, 'id_categoria' )->dropDownList($listCategorias)->label(false) ?>
							
						</div>
                           
<?php 
if($model->idCategoria->categoria == 'Individual'){
  echo
   
						
						
						'<div class="col-sm-6"> <label>Sala</label>'
    
    .$form->field($model, 'id_sala' )->dropDownList($listData)->label(false)
  .'</div>';
...
?>
    ...
    <?= $form->field($model, 'id_modalidade')->dropDownList($listModalidade)->label(false) ?>  </div>
